...
- Build its element following the existing `createMarkerElement` recipe ([`Map.tsx:69-87`](../../web/src/components/Map.tsx)) — a new factory (e.g. `createUserLocationElement`) or a `.map-marker--user` variant — but with a **visually distinct** treatment from the hazard variants (`--fire` = lime, `--flood` = sky). Recommended: a person/dot glyph in a neutral or brand-accent color with a soft "approximate area" halo rather than a sharp pinpoint, to signal imprecision.
...
- **R3 — Visual collision with a nearby hazard marker.** If the user's location coincides with an event, markers may overlap. *Mitigation:* distinct z-order/size for the user marker; acceptable overlap for the prototype, revisit if it obscures events.
- **R4 — Scope creep toward GPS / nearby-events panel.** *Mitigation:* Out-of-Scope draws the line; those are separate proposals.

## Verification Plan

1. Unit/component (vitest + Testing Library): with a mocked `['context']` query returning a location, assert one `.map-marker--user` (or equivalent) is present at the expected coords; with null location, assert none.

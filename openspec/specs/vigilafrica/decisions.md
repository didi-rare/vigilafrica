...
- Subject: `[VigilAfrica] Ingestion failed at {time}` *(superseded 2026-05-22 by [chore-alert-env-label](../../archive/proposal-chore-alert-env-label.md); the active subject is `[VigilAfrica:<env>] Ingestion failed for <country> at <time>` where `<env>` is `staging`, `production`, `local`, `demo`, or `unknown`)*
